...
struct Region {
	int x0, x1, y0, y1;
	explicit Region(int x0, int x1, int y0, int y1) : x0(x0), x1(x1), y0(y0), y1(y1) {}
	void print() {
...
void render(int w, int h, int samps, Ray cam, Vec cx, Vec cy, Vec *c, const Region reg) {
	int y0 = reg.y0, y1 = reg.y1;
	int x0 = reg.x0, x1 = reg.x1;

	for (int y = y0; y < y1; y++) { // Loop over image rows
		for (unsigned short x = x0, Xi[3] = {0, 0, static_cast<unsigned short>(y * y * y)}; x < x1;
			 x++) { // Loop cols
...
std::pair<size_t, size_t> usage(int argc, char *argv[], size_t w, size_t h) {
	// read the number of divisions from the command line
	if (!((argc == 1) || (argc == 3))) {
		std::cerr << "Invalid syntax: smallpt_thread_pool <width_divisions> <height_divisions>"
				  << std::endl;
		exit(1);
	}
